...
from thirdparty.CmsVulScan.modules import globalVar as gl
from thirdparty.CmsVulScan.modules.banner import banner
from thirdparty.CmsVulScan.CmsPay.merge_pay import generate
print(banner())
# 初始化全局变量
gl._init()
class main():
    def __init__(self,url,Payload_Dict,Clas,ages):
        # 初始化url
        url = Url_init(url)
...
def Interface(u):
    ...
    if ages.file == None:
        main(ages.url,Payload_Dict,Clas,ages)
    else:
        file = open(ages.file,'r',encoding='utf-8')
        for i in file.readlines():
            url = i.replace("\n","")
            main(url,Payload_Dict,Clas,ages)
