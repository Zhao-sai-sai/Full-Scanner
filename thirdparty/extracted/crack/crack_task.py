...
import thirdparty.extracted.logs.log as Log
from thirdparty.extracted.parse.parser import Parser
    ...
    def run(self, id, url):
        ...
        try:
            ...
            if username and password:
                Log.Info(f"[*] 正在重新检查... {url} {username} {password}")
                recheck_flag = self.recheck(username, password)
                if recheck_flag:
                    Log.Success(f"[+] 成功: {url}  {username}/{password}")
                    return
                else:
                    Log.Info(f"[-] 重新检查失败: {url}  {username}/{password}")
            Log.Error("[-] 失败的: " + url)
